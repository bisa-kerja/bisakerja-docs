...
This site uses Algolia DocSearch through `@docsearch/docusaurus-adapter`, with config defined under `themeConfig.docsearch` in `docusaurus.config.ts`.
...
Optional Ask AI vars:

- `DOCSEARCH_ASK_AI_ASSISTANT_ID`
- `DOCSEARCH_ASK_AI_SUGGESTED_QUESTIONS`

If required values are missing, site still builds and search UI stays hidden.
...
Use search-only API key for `DOCSEARCH_API_KEY`, not Admin API key.

Full setup for local, Netlify, Ask AI, suggested questions, and troubleshooting lives in:

- `docs/operations/search-and-ask-ai.mdx`
