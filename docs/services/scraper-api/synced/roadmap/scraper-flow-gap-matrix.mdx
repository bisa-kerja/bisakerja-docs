...
| 6    | Batch AI processing                                | Enrichment stage uses configured batch/max-attempt settings and retries retryable provider failures | implemented | Data ingestion owner                    | `src/config/settings.py`, `src/modules/enrichment/service.py`, `src/cli/pipeline.py`                 | Dedicated queue worker for decoupled enrichment throughput remains follow-up work |
| 7    | Extract skills and structure requirements          | OpenAI structured output and source fallback both wired into stage enrich execution                 | implemented | Data ingestion owner                    | `src/modules/enrichment/**`, `src/integrations/ai/**`, `src/cli/pipeline.py`                         | Quality tuning for prompt coverage remains iterative work                         |
| 8    | Insert enriched results to local structured tables | Enrichment persists `job_skills_staging`, `job_requirements_staging`, and AI request logs           | implemented | Data ingestion owner                    | `src/modules/enrichment/repositories.py`, `src/modules/persistence/models.py`, `src/cli/pipeline.py` | No critical gap for baseline staging persistence                                  |
| 9    | Upsert to main Backend DB                          | Backend sync client and sync event audit exist                                                      | partial     | Data ingestion owner with backend owner | `src/integrations/backend/client.py`, `src/modules/sync/**`                                          | Final backend schema resolver and live contract tests remain follow-up work       |
